...
window.addEventListener('load', () => {
        ...
        return new Promise(resolve => {
            ...
            allImagePaths.forEach(path => {
                ...
                img.onerror = () => {
                    ...
                    loadedCount++; imagesLoadedCount = loadedCount;
                    if (loadedCount === totalImages) resolve(); // Résout même en cas d'erreur pour ne pas bloquer
                };
            });
        });
    }
    ...
    function gameLoop(currentTime) { 
        ...
        if (player) {
            player.update(); // Joueur
             ...
             console.error("Player n'est pas défini ! Problème de chargement d'image ?");
             endGame(); return;
        }
        ...
        for (let i = powerUps.length - 1; i >= 0; i--) { 
            ...
            if (player && player.x < pu.x + pu.w && player.x + player.w > pu.x && player.y < pu.y + pu.h && player.y + player.h > pu.y) { 
        ...
        for (let i = collectibles.length - 1; i >= 0; i--) { 
            ...
            if (player && player.x < coll.x + coll.w && player.x + player.w > coll.x && player.y < coll.y + coll.h && player.y + player.h > coll.y) { 
                 updateScore(10); 
    ...
    function updateScore(value = 1) { 
        // Ne calcule le score que si le jeu est en cours
        ...
        // Vérifie si on peut débloquer le prochain bonus
    ...
    async function handleInput(e) { 
        ...
        if (isGameOver) {
            ...
            if (player) player.jump(); // Ne saute que si le joueur existe
        }
    }
